...
static const PegInfo zmqfbstats_peg_names[] =
{
    ...
    { CountType::SUM, "zmq_event_count", "number of events processed by zmqfb logger"},
...
static void alert_event(Packet* p, const char*, ZmqFbConfig* config, const Event* event)
{
    ...
    if ( p && p->ptrs.ip_api.is_ip() ) {
        ...
        action = p->active->get_action_string();
        ips = snort::get_ips_policy();
        if ( ips != nullptr ) {
            memcpy(uuid, ips->uuid, sizeof(uuid_t));
        }
    }

    auto action_str = fbBuilder.CreateString(action);
    ...
    std::vector<uint8_t> policy_uuid_vector((uint8_t*) uuid, ((uint8_t*) uuid) + sizeof(uuid_t));
    auto IntrusionPolicyUUID = fbBuilder.CreateVector(policy_uuid_vector);
    ...
    if ( p )
    {
        eventBuilder.add_IntrusionPolicyUUID(IntrusionPolicyUUID);
        ...
        eventBuilder.add_Action(action_str);
        zmqfbstats.zmq_event_count++;
    }

    auto fbb = eventBuilder.Finish();
    fbBuilder.Finish(fbb, "INTS");
